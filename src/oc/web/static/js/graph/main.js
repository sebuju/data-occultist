...
function _refKey() {
    ...
    return JSON.stringify([
        ...
        (P.actions || []).map((x) => [x.id, x.sources, x.dest, Object.keys(x.slots || {}),
            Object.entries(x.reg_ops || {}).map(([rid, o]) => [rid, o.op, o.dest, o.keys,
                (o.writes || []).map((w) => [w.key, w.remove])])]),
        ...
        (P.gates || []).map((g) => [g.id, g.source, g.targets, (g.conds || []).length, g.logic, g.negate, g.enabled]),
