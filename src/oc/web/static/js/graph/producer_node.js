...
// bare checkbox (for grid rows whose labCell already names it — avoids a doubled label)
...
// resolver. Returned as flat [label, control, ...] pairs to spread into the body grid.
...
export function producerParts(pn, cols = [], free = []) {
    ...
    const body = frag(
        ...
        // frag, so dropping it straight into the grid leaks 2 loose cells per filter and shifts
        // every row below off the 2-col grid. The section add-button lives in the label.
