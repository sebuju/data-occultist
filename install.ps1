<#
  install.ps1 - one-stop setup for oc on Windows.
...
function New-AppShortcut {
  ...
  $ico = Join-Path $root 'assets\oc.ico'
  if (-not (Test-Path $pyw)) { return }
  $dirs = @(
    [Environment]::GetFolderPath('Desktop'),
    (Join-Path ([Environment]::GetFolderPath('Programs')) 'oc')
  )
  $ws = New-Object -ComObject WScript.Shell
  $made = @()
  foreach ($d in $dirs) {
    New-Item -ItemType Directory -Force -Path $d | Out-Null
    $lnk = $ws.CreateShortcut((Join-Path $d 'oc.lnk'))
    $lnk.TargetPath = $pyw
    $lnk.Arguments = '-m oc.desktop_main'
    $lnk.WorkingDirectory = $root
    if (Test-Path $ico) { $lnk.IconLocation = $ico }
    $lnk.Description = 'oc - teaching UI (desktop)'
    $lnk.Save()
    $made += $d
  }
...
$doShortcut = Ask 'Create an "oc" app shortcut on Desktop + Start menu?' $false
...
if (-not $pyCmd) {
  ...
  Info 'installing oc (dev + desktop extras) ...'
...
if (-not $hasWV2 -and -not $doWebView2) { $gaps.Add('Edge WebView2 Runtime (browser UI via oc teach still works without it)') }
if ($doShortcut -and $pyCmd -and -not $gaps.Count) { New-AppShortcut }

# === summary =================================================================
Write-Host ''
if ($gaps.Count -eq 0) {
  Ok 'done. Launch the app via the "oc" shortcut (if created), or:'
  Write-Host '    .\.venv\Scripts\Activate.ps1'
  Write-Host '    oc teach          # browser UI at http://127.0.0.1:8000'
  Write-Host '    oc app            # server + native window (from a terminal)'
  Write-Host '    oc view           # native window onto an already-running server'
  exit 0
} else {
