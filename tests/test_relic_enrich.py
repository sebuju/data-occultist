...
# The relic producer keys (name, item); write and read must agree (in production this
# comes from the dataset's key_map_for) so a re-keyed read doesn't collapse rows.
_KEY = KeySpec(fields=("name", "item"))
...
def test_reward_rows_flatten_per_relic_reward_dedup_state():
    rows = relic.relic_reward_rows(_build())
    # 3 distinct rewards; state/chance dropped, so the Intact+Radiant Nikana collapse to one row
    assert len(rows) == 3
    by_item = {r["item"]: r for r in rows}
    assert by_item["Akstiletto Prime Barrel"] == {
        "name": "AXI A1", "item": "Akstiletto Prime Barrel", "rarity": "Uncommon",
        "ducats": 25}
    # untradeable Forma falls back to 0 ducats
    assert by_item["Forma Blueprint"]["ducats"] == 0
    # reward shared across states appears once, with no chance/state columns
    assert by_item["Nikana Prime Blueprint"] == {
        "name": "AXI A1", "item": "Nikana Prime Blueprint", "rarity": "Rare", "ducats": 15}
...
def test_producer_writes_keyed_rows_to_dataset(tmp_path, monkeypatch):
    ...
    keys = {(r["name"], r["item"]) for r in rows}
    assert ("AXI A1", "Nikana Prime Blueprint") in keys   # Intact+Radiant collapse to one
    assert len(rows) == 3
